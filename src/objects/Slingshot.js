...
export default class Slingshot {
	...
	update(dt) {
		...
		[...this.animations].forEach((entity) => entity.update(dt));
	...
	render() {
		...
		this.bird.render();
		[...this.animations].forEach((entity) => entity.render());
	...
	unload() {
		...
		this.animations.push(new Explosion(this.bird.body.position.x, this.bird.body.position.y));
		this.bird.update();
		this.bird = null;
	}
